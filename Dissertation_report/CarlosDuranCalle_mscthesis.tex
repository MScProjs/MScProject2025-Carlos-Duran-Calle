...
Six socio-demographic factors were subjected to linear regression analysis in order to measure their associations with student outcomes. Proportional distributions among the three outcome classes (pass, fail, and withdrawn) were examined, and $R^2$ coefficients and slope measurements were used to evaluate the relevance of the variables. Table~\ref{tab:var_selection} displays a summary of the regression's results with $R^2$ and the corresponding slope for each class and variable selection.
...
The three-class imbalanced nature of student outcome prediction necessitates specialised metrics that emphasise early identification of at-risk students. This led to the development of custom dropout-focused scoring functions to supplement a baseline evaluation framework of seven traditional $sklearn$ metrics that provide balanced perspectives on classification performance across precision, recall, and F1-score dimensions. Standard ML evaluation metrics prioritise overall classification accuracy. Table~\ref{tab:std_metrics} provides an appreciation of these standard metrics.

To separate performance measurement for the minority withdrawal class (Class 0) and at-risk student populations in student dropout prediction, three specialised scoring functions were created. Its addition is also visible in the Algorithm~\ref{alg:custom_dropout_scorers}, where the ML model to be used for training is the input estimator.
...
\begin{algorithm}[h]
	\KwIn{$estimator$, $X$, $y$}
	\KwOut{Custom scoring functions for dropout and at-risk students}
...
\vspace{1cm} 
\noindent
This chapter presents a detail analysis comparing six machine learning models (RF, LR, KNN, LightGBM, SVM, and NN) for student dropout prediction using the OULAD dataset, with primary focus on optimizing dropout recall performance to enable effective early intervention strategies.
...
In contrast to dropout recall, at-risk recall (which merges the Withdrawn and Fail classes into one) exhibited distinct rankings.  The highest at-risk recall ($40.8\%$) was attained by LightGBM, which was closely followed by SVM ($39.2\%$), RF ($39.3\%$), and Logistic Regression ($39.6\%$).  This metric demonstrated the models' ability to detect students who need more academic assistance than just the immediate risk of dropping out.

Overall classification balance across all outcome categories was revealed by weighted F1 scores.  The highest weighted F1 score ($55.2\%$) was attained by NN, which was followed by KNN ($50.9\%$) and RF ($49.5\%$).  These findings showed that dropout recall-optimized models inevitably compromised overall classification balance, which is consistent with the intentional design decision to give dropout student identification priority.
